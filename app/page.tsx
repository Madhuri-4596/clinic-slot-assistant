  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Main navigation">{[
        ['board','grid','Overview'],['waitlist','users','Waitlist'],['activity','clock','Call activity'],['setup','settings','CALL-E connection'],
    ...
    <main>
      <header className="topbar"><div className="breadcrumb">Workspace <Icon name="chevron" size={13}/><strong>{{board:'Overview',waitlist:'Waitlist',activity:'Call activity',setup:'CALL-E connection'}[tab]}</strong></div><div className="top-actions"><span className="badge demo"><span/>Demo data</span><button aria-label="How it works" className="text-button" onClick={()=>setHelp(true)}><Icon name="info" size={17}/><span>How it works</span></button></div></header>
      <div className="page-wrap">
        ...
        {!data ? <div className="loading"><span className="spinner"/>Getting your workspace ready…{error&&<button className="primary" onClick={()=>void load()}>Try again</button>}</div> : <>
          {tab==='board'&&<>
            ...
            <div className="demo-note"><Icon name="info" size={16}/><span>This is a working prototype with fictional people and appointments. Demo calls are simulated.</span><button onClick={()=>setTab('setup')}>Connection details <Icon name="arrow" size={15}/></button></div>
